...
class GiftManagementApp {
    ...
    async loadEmployeeOptions() {
        ...
        try {
            // Use the colleagues endpoint to get the list of available colleagues
    ...
    loadEmployeeOptionsFromCache() {
        ...
        if (this.currentUser.role === 'employee') {
            // Employees can only transfer to other employees (not managers)
    ...
    async switchTab(tabName) {
        ...
        // If switching to transfer tab, refresh employee options
        if (tabName === 'transfer') {
            await this.loadEmployeeOptions();
        }
    }
